    ...
    <div className="min-h-screen bg-gray-100">
      ...
      <header className="bg-orange-500">
        <div className="max-w-7xl mx-auto px-4">
          <div className="flex items-center gap-4 py-3">
            <div className="text-white text-2xl font-bold whitespace-nowrap">ucuzuygun.com</div>
            <div className="flex flex-1">
              <input type="text" placeholder="Ürün, marka veya kategori ara..." className="flex-1 px-4 py-2 rounded-l-md outline-none text-gray-800" />
              <button className="bg-yellow-400 px-4 py-2 rounded-r-md font-bold text-gray-800">Ara</button>
            </div>
            <div className="flex gap-6 text-white text-sm">
              <button className="flex flex-col items-center"><span className="text-xl">♡</span><span>Favoriler</span></button>
              <button className="flex flex-col items-center"><span className="text-xl">🛒</span><span>Sepet</span></button>
              <button className="flex flex-col items-center"><span className="text-xl">👤</span><span>Hesabım</span></button>
            </div>
          </div>
          <nav className="flex gap-1 overflow-x-auto pb-1">
            {["Elektronik", "Giyim", "Ev & Yaşam", "Kozmetik", "Spor", "Kitap", "Oyuncak", "Süpermarket", "Tüm Kategoriler"].map((cat) => (
              <a key={cat} href={`/kategori/${cat.toLowerCase()}`} className="text-white text-xs px-3 py-2 whitespace-nowrap hover:bg-orange-600 rounded">
                {cat}
              </a>
            ))}
          </nav>
      ...
      <div className="bg-gradient-to-r from-orange-500 to-orange-300 py-12 px-8">
        <div className="max-w-7xl mx-auto">
          ...
          <button className="bg-white text-orange-500 font-bold px-6 py-2 rounded">Şimdi Keşfet</button>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 py-6">
        ...
        <div className="flex gap-4 overflow-x-auto pb-2">
          {[
            ...
            <a key={cat.label} href={`/kategori/${cat.slug}`} className="flex flex-col items-center gap-2 min-w-16 cursor-pointer">
              <div className="w-14 h-14 rounded-full bg-orange-100 flex items-center justify-center text-2xl hover:bg-orange-200 transition">
                {cat.icon}
              </div>
      ...
      <div className="max-w-7xl mx-auto px-4 py-4">
        ...
        <div className="flex gap-4 overflow-x-auto pb-2">
          {[
            ...
            <div key={store.name} className="flex flex-col items-center gap-2 min-w-28 bg-white rounded-lg border border-gray-200 p-3 cursor-pointer hover:-translate-y-1 transition-transform">
              <div className={`w-11 h-11 rounded-full ${store.color} flex items-center justify-center text-white font-bold`}>{store.initials}</div>
